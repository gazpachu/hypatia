...
//	}
	...
	enrolConfirmation() {
		this.setState({ modalTitle: CONSTANTS.CONFIRM_ENROL }, () => {
	...
	handleChange(event) {
		...
		if (event.target.checked) selectedSubjects.push(event.target.value);
			...
			for (let i = 0; i < selectedSubjects.length; i += 1) {
				if (selectedSubjects[i] === event.target.value) index = i;
			}
	...
	render() {
		...
		let subjects = null;
		let totalCredits = 0;
		let section = this.props.location.pathname.substr(this.props.location.pathname.lastIndexOf('/') + 1);

		if (isLoaded(this.props.course) && isLoaded(this.props.files) &&
		!isEmpty(this.props.course) && !isEmpty(this.props.files)) {
			Object.keys(this.props.course).map((key) => {
				courseID = key;
		...
		if (course && course.subjects && isLoaded(this.props.subjects) &&
		!isEmpty(this.props.subjects) && isLoaded(this.props.users) && !isEmpty(this.props.users)) {
		...
		return (
			<section className="page course">
				{course ? <div className="page-wrapper">
					...
					<div className="meta">
						<Icon glyph={Level} />
						{this.props.levels[course.level].title} ({this.props.levels[course.level].code}) ({totalCredits} Credits)
						<Icon glyph={Calendar} />Enrollment from <span className="date">{moment(course.startDate).format('D MMMM YYYY')}</span> until <span className="date">{moment(course.endDate).format('D MMMM YYYY')}</span>
					...
					<ul className="horizontal-nav">
						<li className={classNames('horizontal-nav-item', { active: section === this.props.params.slug })}>
							<Link to={`/courses/${course.slug}`}>Summary</Link>
						</li>
						<li className={classNames('horizontal-nav-item', { active: section === 'subjects', hidden: !subjects })}>
							<Link to={`/courses/${course.slug}/subjects`}>Subjects</Link>
						</li>
						<li className={classNames('horizontal-nav-item', { active: section === 'fees' })}>
							<Link to={`/courses/${course.slug}/fees`}>Fees</Link>
						</li>
						<li className={classNames('horizontal-nav-item', { active: section === 'requirements' })}>
					...
					<div className={classNames('columns', { 'single-column': (!course.content2 && !course.content2), hidden: (section !== this.props.params.slug) })}>
						<div className="column page-content">
							{featuredImage ? <img className="featured-image" role="presentation" src={featuredImage.url} /> : ''}
